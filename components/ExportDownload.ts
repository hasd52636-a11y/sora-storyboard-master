...
export async function downloadStoryboardSheet(element: HTMLElement, filename: string): Promise<void> {
  try {
    ...
    const originalBackgroundFilters: Map<HTMLElement, string> = new Map();
    ...
    allImages.forEach(img => {
      ...
      if (img === referenceImageElement) {
        ...
        // 对其他所有图片应用灰度和对比度
        // 使用更强的过滤器确保html2canvas能捕获
        img.style.filter = 'grayscale(100%) contrast(1.3) brightness(1.05)';
      }
    });
    
    // 也对图片容器应用过滤器，确保背景也被处理
    const imageContainers = element.querySelectorAll('[class*="aspect-video"]') as NodeListOf<HTMLElement>;
    imageContainers.forEach(container => {
      // 检查这个容器是否包含参考图片
      const hasRefImage = container.querySelector('[style*="border-dashed"][style*="border-red"]');
      if (!hasRefImage) {
        originalBackgroundFilters.set(container, container.style.filter || '');
    ...
    originalBackgroundFilters.forEach((originalFilter, container) => {
