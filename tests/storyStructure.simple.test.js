...
function calculateStoryStructure(frameCount, duration) {
...
function getStoryStageForFrame(frameIndex, config) {
...
function calculateFrameTimeInfo(frameIndex, config) {
...
function getAllFrameTimeInfo(config) {
...
function validateStoryStructure(config) {
    const errors = [];
    ...
    testCases.forEach(frameCount => {
        ...
        if (frameCount >= 4) {
            if (config.setupFrames < 1)
                throw new Error(`Setup frames must be at least 1`);
            if (config.buildFrames < 1)
                throw new Error(`Build frames must be at least 1`);
            if (config.climaxFrames < 1)
                throw new Error(`Climax frames must be at least 1`);
            if (config.resolutionFrames < 1)
                throw new Error(`Resolution frames must be at least 1`);
        }
    ...
    testCases.forEach(({ frames, duration }) => {
        ...
        frameTimeInfos.forEach((info, index) => {
            ...
            if (Math.abs(info.timeStart - expectedStart) > 0.01)
                throw new Error(`Frame ${index} start time mismatch`);
            if (Math.abs(info.timeEnd - expectedEnd) > 0.01)
                throw new Error(`Frame ${index} end time mismatch`);
        });
...
function testFrameAllocationCorrectness() {
    ...
    testCases.forEach(frameCount => {
        ...
        if (sum !== frameCount)
            throw new Error(`Sum of stages does not equal total`);
