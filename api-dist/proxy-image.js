...
async function handler(request) {
    ...
    if (!apiKey) {
        return withCORSHeaders(new node_fetch_1.Response(JSON.stringify({ error: 'Internal server error: API key not configured' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
        }));
    }
    try {
        ...
        if (!prompt) {
            return withCORSHeaders(new node_fetch_1.Response(JSON.stringify({ error: 'Missing required parameter: prompt' }), {
                status: 400,
                headers: { 'Content-Type': 'application/json' },
            }));
        }
        ...
        if (!siliconFlowResponse.ok) {
            ...
            // 成功响应
            const responseBody = await siliconFlowResponse.text();
            return withCORSHeaders(new node_fetch_1.Response(responseBody, {
