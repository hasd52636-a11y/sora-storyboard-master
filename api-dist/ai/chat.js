...
async function handler(req) {
    ...
    if (!KEY) {
        // 4. 未授权返回中添加 CORS 头
        return withCORSHeaders(new node_fetch_1.Response(JSON.stringify({ error: 'Authorization required: Missing X-SF-Key in headers.' }), {
            status: 401,
            headers: { 'Content-Type': 'application/json' }
        }));
    }
    ...
    try {
        ...
        return withCORSHeaders(new node_fetch_1.Response(JSON.stringify(data), {
            status: 200,
            headers: { 'Content-Type': 'application/json' }
        }));
    }
